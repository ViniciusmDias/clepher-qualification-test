...
export function Search({ onSelect, setCompanyName }: SearchProps){
  ...
  const handleSearch = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (event.target.value.length > 0) {
      try {
        const response = await fetch(`https://www.alphavantage.co/query?function=SYMBOL_SEARCH&keywords=${event.target.value}&apikey=${process.env.REACT_APP_ALPHA_VANTAGE_API_KEY}`);
